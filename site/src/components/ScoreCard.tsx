...
interface ScoreCardProps { hackathonSlug: string; trackSlug: string; criteria: Criterion[]; lang: 'zh' | 'en'; }
...
export function ScoreCard({ hackathonSlug, trackSlug, criteria, lang }: ScoreCardProps) {
  ...
  return (
    <div className="rounded-lg border border-secondary-bg bg-dark-bg p-6">
      <h3 className="text-lg font-heading font-bold text-white mb-6">{t('评分卡', 'Score Card')}</h3>
      <div className="mb-6">
          ...
          return (
            <div key={idx} className="border-b border-secondary-bg pb-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-xs text-muted">{t('权重', 'Weight')}: {(c.weight * 100).toFixed(0)}%</span>
              </div>
              ...
              <textarea value={comments[idx]} onChange={e => updateComment(idx, e.target.value)} placeholder={t('评语（可选）', 'Comment (optional)')} className="w-full bg-surface border border-secondary-bg rounded-md px-3 py-2 text-white text-sm resize-none h-16 focus:border-lime-primary focus:outline-none" />
            </div>
          );
        })}
      </div>
      <div className="mt-6">
        ...
        <textarea value={overall} onChange={e => setOverall(e.target.value)} placeholder={t('整体评语...', 'Overall comment...')} className="w-full bg-surface border border-secondary-bg rounded-md px-3 py-2 text-white text-sm resize-none h-24 focus:border-lime-primary focus:outline-none" />
      </div>
      <div className="mt-6 p-4 rounded-lg border border-secondary-bg bg-surface">
        <label className="flex items-start gap-3 cursor-pointer">
          ...
          <span className="text-sm text-light-gray">{t('我确认与该团队无利益冲突关系', 'I confirm no conflict of interest with this team')}</span>
        </label>
      </div>
