...
- **G-4**: 建立结构化的 Hacker Profile 系统，支撑开发者网络和组队发现
- **G-5**: 集成 AI Agent 辅助项目评审摘要、自动化评分建议
...
│   └── _schema.yml
│
├── teams/                               # 队伍数据（PR-based 组队）
│   ├── team-alpha/
│   │   └── team.yml                     # 队伍配置（Schema: teams/_schema.yml）
│   └── _schema.yml
│
├── .github/
│   ├── workflows/
...
│   │   ├── validate-register.yml        # 校验报名 Issue（Profile 存在 + 资格）
│   │   ├── validate-nda.yml             # 校验 NDA 签署 Issue
│   │   ├── validate-score.yml           # 校验评分 Issue 格式
│   │   ├── sync-issue-data.yml          # Issue 数据 → YAML PR（报名/NDA 持久化）
│   │   ├── ai-review.yml
│   │   ├── status-update.yml
│   │   └── upload-assets.yml
│   ├── ISSUE_TEMPLATE/
...
hacker:
  ...
  team: "team-alpha"                      # 当前所属队伍 slug（全局一人一队）
                                           # 由 sync-team-data.yml 写入，退出或解散时清除
...
project:
  ...
  team_ref: "team-alpha"                  # 引用 teams/{slug}/team.yml（成员信息在 team.yml 管理）
...
| 队伍成员 | `team.members[].role` | `developer` \| `product` \| `designer` \| `marketing` \| `researcher` | team.yml 中的成员角色（Hacker 细分） |
...
```
                    ...
                    │                                               │
                    ├─── 报名 ─────────────────────────────────────┤
                    │   站点表单 → Issue [Register] username - slug  │
                    │   → validate-register.yml 检查 Profile + 资格  │
                    │   → Label: registered                         │
                    │   → sync-issue-data.yml 持久化到 profile YAML  │
                    │   → (NDA 场景) 提示签署 NDA Issue              │
                    │                                               │
                    ├─── 项目提交 ──────────────────────────────────┤
...
**Description:** 参赛者在活动页面填写报名表单，站点创建 [Register] Issue。`validate-register.yml` 校验 Profile 存在和资格，通过后添加 `registered` Label。`sync-issue-data.yml` 将报名数据持久化到 profile YAML 并创建 PR。NDA 场景下提示用户创建 [NDA] Issue，`validate-nda.yml` 校验后添加 `nda-approved` Label，`sync-issue-data.yml` 同步签署记录到 YAML。

**Acceptance Criteria:**
- [ ] 活动页面报名表单（RegisterForm）创建 [Register] Issue
- [ ] `validate-register.yml` 检查用户 Profile 是否存在
- [ ] `validate-register.yml` 检查 eligibility 约束（学生身份等）
- [ ] 校验通过后 Issue 获得 `registered` Label
- [ ] `sync-issue-data.yml` 在 Label 添加时触发，将报名数据写入 profile YAML 并创建 PR
- [ ] NDA 活动：报名后提示签署 NDA Issue（NDASignForm 创建 [NDA] Issue）
- [ ] `validate-nda.yml` 校验 NDA Issue，通过后添加 `nda-approved` Label
- [ ] `sync-issue-data.yml` 将 NDA 签署记录持久化到 profile YAML
...
- **FR-11**: Issue Template 覆盖报名/NDA/评分/申诉；组队改为 PR-based teams/ 目录
- **FR-12**: 赛道双报支持（独立子目录）
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
├── NDA 签署流程增强（Functions presigned URL 鉴权下载，基础 Issue 流程已在 P0 实现）
